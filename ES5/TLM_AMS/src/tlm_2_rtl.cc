...
{
  std::cout<<"[ TLM_2_RTL ] :" << sc_simulation_time()<<" - "<<name()<<" - b_transport"<<std::endl;
  wait(0, SC_NS);
  ...
  case tlm::TLM_WRITE_COMMAND:
	   ...
	   begin_write.notify();    
	   std::cout<<"[ TLM_2_RTL ] write notified"<<std::endl;
	   wait(end_write);    
  break;
  ...
  case tlm::TLM_READ_COMMAND:
	   ...
	   trans.set_response_status(tlm::TLM_OK_RESPONSE);
	   begin_read.notify();
	   std::cout<<"[ TLM_2_RTL ] read notified"<<std::endl;
	   wait(end_read);
  ...
  std::cout<<"[ TLM_2_RTL ] "<< sc_simulation_time()<<" - "<<name()<<" - b_transport ended"<<std::endl;
}
...
while (true) {
    ...
    std::cout<<"[ TLM_2_RTL ] WRITING "<< ioDataStruct.r<<std::endl;
	std::cout<<"[ TLM_2_RTL ] "<< sc_simulation_time() <<" - "<<name()<<" - notify received"<<std::endl;
...
while (true) {
	...
	std::cout << "[ TLM_2_RTL ] READING "<< y.read() << std::endl;
